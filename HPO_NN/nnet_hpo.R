library(dismo) # k-fold
library(nnet) 

# mydata <- read.csv("D:/dataset(prep)/bloodtrans.csv", header = T, row.names = 1)
# mydata <- read.csv("D:/dataset(prep)/churn.csv", header = T)
# mydata <- read.csv("D:/dataset(prep)/eegeyestate.csv", header = T, row.names = 1)
# mydata <- read.csv("D:/dataset(prep)/glass_identification.csv", header = T, row.names = 1)
mydata <- read.csv("D:/dataset(prep)/balancescale.csv", header = T, row.names = 1)
mydata$class <- factor(mydata$class)  # transform 'class' to factor type
...
err_g <- apply(design_grid, 1, nnet_cv, train = mydata, nfold = 5)
min(err_g)
...
err_a <- apply(AD_design, 1, nnet_cv, train = mydata, nfold = 5)
min(err_a)
...
err_d <- apply(DD_design, 1, nnet_cv, train = mydata, nfold = 5)
min(err_d)
...
err_l <- apply(LD_design, 1, nnet_cv, train = mydata, nfold = 5)
